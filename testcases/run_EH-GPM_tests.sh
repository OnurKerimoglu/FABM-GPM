...
rootdir='2019_05_22_PPZZ_NoSi_odemet2_n20_2013-2014_realwind'
run=1
...
if [ $v == 1 ]; then
 ...
 python plot_1D-EH-GPM_PPZZ.py ${rootdir}/fixstochl/1D-40m_GPM-EH_fixstochl_dm.nc 1 1 2
 ...
 python plot_1D-EH-GPM_PPZZ.py ${rootdir}/fixsto_empchl/1D-40m_GPM-EH_fixsto_empchl_dm.nc 1 1 2
 tail -n17 ${rootdir}/fixsto_empchl/1D-40m_GPM-EH_fixsto_empchl_log.txt | head -n9
elif [ $v == 3 ]; then
 echo -e "\nfixsto_G97iChl:"
 if [ $run == 1 ]; then
  ln -sf fabm-EH_AP_AS-GPM_P_Z_fixedSt_G97iChl.yaml fabm.yaml 
  ln -sf output_fixsto_G97iChl.yaml output.yaml
  #ln -sf restart_fixsto_G97iChl_20140101.nc restart.nc
  gotm_fabm_onur >1D-40m_GPM-EH_fixsto_G97iChl_err.txt 2>1D-40m_GPM-EH_fixsto_G97iChl_log.txt
  mkdir -p ${rootdir}/fixsto_G97iChl
  mv 1D-40m_GPM-EH_fixsto_G97iChl* ${rootdir}/fixsto_G97iChl
  #mv restart.nc ${rootdir}/fixsto_G97iChl/restart_fixto_G97iChl_20140101.nc
  cp fabm-EH_AP_AS-GPM_P_Z_fixedSt_G97iChl.yaml ${rootdir}/fixsto_G97iChl/
  echo "fixsto_G97iChl: stored the results in: ${rootdir}/fixsto_G97iChl"
 fi
 python plot_1D-EH-GPM_PPZZ.py ${rootdir}/fixsto_G97iChl/1D-40m_GPM-EH_fixsto_G97iChl_dm.nc 1 1 2
 tail -n17 ${rootdir}/fixsto_G97iChl/1D-40m_GPM-EH_fixsto_G97iChl_log.txt | head -n9
elif [ $v == 4 ]; then
 echo -e "\nvarsto:"
 if [ $run == 1 ]; then
 ...
 python plot_1D-EH-GPM_PPZZ.py ${rootdir}/varsto/1D-40m_GPM-EH_varsto_dm.nc 1 1 2
